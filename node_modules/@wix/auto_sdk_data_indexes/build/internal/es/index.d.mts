...
import { Index, ListIndexesOptions, ListIndexesResponse, ListAvailableIndexesResponse } from './index.typings.mjs';
export { CreateIndexRequest, CreateIndexResponse, DropIndexRequest, DropIndexResponse, Environment, EnvironmentWithLiterals, Failure, Field, IndexSource, IndexSourceWithLiterals, ListAvailableIndexesRequest, ListIndexesRequest, Order, OrderWithLiterals, Paging, PagingMetadata, Status, StatusWithLiterals } from './index.typings.mjs';
...
interface CreateIndexSignature {
     ...
     */
    (dataCollectionId: string, index: NonNullablePaths<Index, `fields` | `fields.${number}.path` | `name`, 4>): Promise<NonNullablePaths<Index, `name` | `fields` | `fields.${number}.path` | `fields.${number}.order` | `status` | `failure.code` | `failure.description` | `unique` | `caseInsensitive` | `source`, 4>>;
}
...
interface ListIndexesSignature {
     ...
     */
    (dataCollectionId: string, options?: ListIndexesOptions): Promise<NonNullablePaths<ListIndexesResponse, `indexes` | `indexes.${number}.name` | `indexes.${number}.status` | `indexes.${number}.failure.code` | `indexes.${number}.failure.description` | `indexes.${number}.unique` | `indexes.${number}.caseInsensitive` | `indexes.${number}.source`, 5>>;
}
/** @internal */
declare function listAvailableIndexes$1(httpClient: HttpClient): ListAvailableIndexesSignature;
interface ListAvailableIndexesSignature {
    /**
     * Lists available indexes limits
     * @param - Data collection to show available indexes for
     */
    (dataCollectionId: string): Promise<NonNullablePaths<ListAvailableIndexesResponse, `regular1Field` | `regular3Field` | `unique1Field`, 2>>;
}
...
/** @internal */
declare const listAvailableIndexes: MaybeContext<BuildRESTFunction<typeof listAvailableIndexes$1> & typeof listAvailableIndexes$1>;

export { Index, ListAvailableIndexesResponse, ListIndexesOptions, ListIndexesResponse, createIndex, dropIndex, listAvailableIndexes, listIndexes };
